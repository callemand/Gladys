...
describe('system.rebootHost', () => {
  it('should reboot the host through systemd-logind (DBus)', async () => {
    const exec = sinon.stub().callsFake((command, callback) => callback(null, { stdout: '', stderr: '' }));
    const { rebootHost } = proxyquire('../../../lib/system/system.rebootHost', {
      child_process: { exec },
    });
    await rebootHost();
    expect(exec.callCount).to.equal(1);
    const command = exec.firstCall.args[0];
  ...
  it('should reject if the DBus command fails', async () => {
    const exec = sinon.stub().callsFake((command, callback) => callback(new Error('dbus-send not found')));
    const { rebootHost } = proxyquire('../../../lib/system/system.rebootHost', {
      child_process: { exec },
    });
    let caughtError;
